...
var file_currency_proto_rawDesc = []byte{
	...
	0x61, 0x74, 0x65, 0x32, 0x48, 0x0a, 0x08, 0x43, 0x75, 0x72, 0x72, 0x65, 0x6e, 0x63, 0x79, 0x12,
	0x3c, 0x0a, 0x07, 0x47, 0x65, 0x74, 0x52, 0x61, 0x74, 0x65, 0x12, 0x15, 0x2e, 0x63, 0x75, 0x72,
...
type CurrencyClient interface {
	GetRate(ctx context.Context, opts ...grpc.CallOption) (Currency_GetRateClient, error)
}
...
func (c *currencyClient) GetRate(ctx context.Context, opts ...grpc.CallOption) (Currency_GetRateClient, error) {
	stream, err := c.cc.NewStream(ctx, &_Currency_serviceDesc.Streams[0], "/currency.Currency/GetRate", opts...)
	if err != nil {
		return nil, err
	}
	x := &currencyGetRateClient{stream}
	return x, nil
}

type Currency_GetRateClient interface {
	Send(*RateRequest) error
	Recv() (*RateResponse, error)
	grpc.ClientStream
}

type currencyGetRateClient struct {
	grpc.ClientStream
}

func (x *currencyGetRateClient) Send(m *RateRequest) error {
	return x.ClientStream.SendMsg(m)
}

func (x *currencyGetRateClient) Recv() (*RateResponse, error) {
	m := new(RateResponse)
...
type CurrencyServer interface {
	GetRate(Currency_GetRateServer) error
}
...
func (*UnimplementedCurrencyServer) GetRate(Currency_GetRateServer) error {
...
func _Currency_GetRate_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(CurrencyServer).GetRate(&currencyGetRateServer{stream})
}

type Currency_GetRateServer interface {
	Send(*RateResponse) error
	Recv() (*RateRequest, error)
	grpc.ServerStream
}

type currencyGetRateServer struct {
	grpc.ServerStream
}

func (x *currencyGetRateServer) Send(m *RateResponse) error {
	return x.ServerStream.SendMsg(m)
}

func (x *currencyGetRateServer) Recv() (*RateRequest, error) {
	m := new(RateRequest)
